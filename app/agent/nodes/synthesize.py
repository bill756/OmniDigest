...
SYNTHESIZE_SYSTEM_PROMPT = """你是一个全能型的内容深度精读与事实核查报告架构师。
你的核心任务是：深入理解提供的文章正文和事实核验断言，严格遵循以下指定的输出格式规范，由你自主进行高密度的内容提炼、逻辑归纳与专业报告合成。

【输出格式约束（纯 JSON 输出）】：
请严格以 JSON 格式输出，包含以下三个字段：
{
  "mindmap": "思维导图 Markdown 文本",
  "summary": "结构化精读摘要 Markdown 文本",
  "final_report": "完整分析长篇报告 Markdown 文本"
}

【各模块具体格式骨架（仅提供格式模板，内容由你根据文章自主填充）】：
        ...
        try:
            ...
            try:
                ...
                if raw_text.startswith("# ") or "## 📌 核心思维导图" in raw_text:
                    ...
                    sm_match = re.search(r"## 💡 (?:深度脱水摘要|核心内容提炼|深度精读摘要)\s*([\s\S]*?)(?=\n---\n|\n## |\Z)", raw_text)
                    summary = sm_match.group(1).strip() if sm_match else ""
                else:
                    raise json_err
        except Exception as e:
            logger.warning(f"LLM 报告合成执行或解析异常，降级到本地备用服务: {e}")
